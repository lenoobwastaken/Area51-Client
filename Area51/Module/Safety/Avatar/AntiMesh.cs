...
{
	internal class AntiMesh : BaseModule, OnObjectInstantiatedEvent
	{
		public AntiMesh()
			: base("Anti Mesh", "Bad Mesh", Main.Instance.Avatarbutton, null, isToggle: true)
		{
		}
		...
		public bool OnObjectInstantiatedPatch(IntPtr assetPtr, Vector3 pos, Quaternion rot, byte allowCustomShaders, byte isUI, byte validate, IntPtr nativeMethodPointer, _AssetManagement.ObjectInstantiateDelegate originalInstantiateDelegate)
		{
			...
			if (flag)
			{
				...
				for (int i = 0; i < list.Count; i++)
				{
					if (!(list[i] == null))
					{
						MunchenAntiCrash.ProcessRenderer(list[i], limitPolygons: true, limitMaterials: false, limitShaders: false, ref previousProcess);
					}
				}
				if (previousProcess.nukedMeshes > 0)
				{
					LogHandler.Log(LogHandler.Colors.Green, $"[AntiCrash] Removed {previousProcess.nukedMeshes} Bad Meshs");
					LogHandler.LogDebug($"[Anti AviCrash] Removed {previousProcess.nukedMeshes} Bad Meshs");
				}
			}
			return true;
		}
