...
{
	internal class CuddleBot1 : BaseModule
	{
		...
		public CuddleBot1()
			: base("Start\nBot One", "Bots Join World", Main.Instance.Privatebotbutton, ButtonIcons.PlayerMenu)
		{
		}
			...
			try
			{
				if (!Directory.Exists("\\Area51\\Bot\\"))
				{
					...
					if (File.Exists(text + "Area51.exe"))
					{
						...
						StartBot(text + "Area51.exe", array[0] + "|" + region + "|" + getWorldID + "|avtr_c4961195-1980-4a98-bb95-3cbe0e063463|" + getUserID + "|0.7|CUDDLECLI|" + keyString);
					}
					else
					{
